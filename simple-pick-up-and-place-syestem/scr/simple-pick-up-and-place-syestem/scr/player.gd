...
func _physics_process(delta: float) -> void:
	...
	if $head/Camera3D/lookat.is_colliding():
		...
		if ActiveObj != collider:
			if ActiveObj != null:
				var original_visual_node = find_node_in_group(ActiveObj, "Pickable Items (Mesh)")
				if original_visual_node != null:
					original_visual_node.set_layer_mask_value(20,false)
			ActiveObj = collider
			var visual_node = find_node_in_group(ActiveObj, "Pickable Items (Mesh)")
			if ActiveObj is RigidBody3D:
				get_node(ObjNameUI).get_node("ObjName").text = '“E" to pick up: ' + ActiveObj.name
				if visual_node != null and pickObj == null:
					visual_node.set_layer_mask_value(20,true)

	else:
		if ActiveObj != null:
			var visual_node = find_node_in_group(ActiveObj, "Pickable Items (Mesh)")
			if visual_node != null:
				visual_node.set_layer_mask_value(20,false)
			ActiveObj = null
...
func _wait_and_resume_sleep(pickObj):
	var timer = Timer.new()
	add_child(timer)
	...
	await timer.timeout
	# 等待结束后，将 can_sleep 设置回 true，允许物体进入休眠状态
	pickObj.can_sleep = true
